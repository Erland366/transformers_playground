...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x, kv_cache=None):
        ...
        wei = relu_softpick_1(wei, dim=-1) # (B, H, T, T)
    ...
    def generate(self, idx, max_new_tokens, temperature=1, use_cache=True):
        if use_cache:
            ...
            for _ in range(max_new_tokens):
                ...
                # apply relu_softpick_1 to get probabilities
                probs = relu_softpick_1(logits, dim=-1) # (B, C)
...
if use_wandb:
    ...
    wandb.init(
        ...
        config=wandb_config,
        name="transformer-lm-animesubs-256seq-256embed-4head-6layer.AMD"
    )
