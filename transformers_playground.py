...
# this script is configured for a CUDA GPU, but should run on AMD ROCm or CPU with fallbacks
...
if rocm_runtime_present() and torch.version.hip is None:
    raise RuntimeError(
        ...
        "Reinstall torch/torchvision/torchaudio from the ROCm index, e.g. "
...
if device.type == 'cpu':
    ...
    ppl_batch_size = 16
    max_new_tokens = 200
    print(f"CPU detected; using reduced settings: seq_len={seq_len}, batch_size={batch_size}, total_steps={total_steps}.")
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x, kv_cache=None):
        ...
        if self.use_flex_attention and T == self.seq_len:
...
model.to(device)
optimizer = torch.optim.AdamW(model.parameters(), lr=2e-3, fused=use_fused_optimizer)
...
ppl, loss = perplexity(model, seq_len, seq_len, batch_size=ppl_batch_size, val_steps=ppl_val_steps)
...
print(decode(model.generate(idx=torch.tensor([idx], dtype=torch.long).to(device), max_new_tokens=max_new_tokens, temperature=0.5, use_cache=True)[0].tolist()))
